...
internal class Day17 : IReadInputDay
{
    (int minX, int maxX, int minY, int maxY) target = (185, 221, -122, -74);
    private int maxHeight;
    private int hits;

    public void ReadData()
    {
        for (var x = 0; x < target.maxX * 2; x++)
            for (var y = -1000; y <= 2000; y++)
                if (WillItHit((x, y), out var height))
                {
    ...
    public object TaskA() => maxHeight;
    public object TaskB() => hits;
    ...
    private bool WillItHit(Vec2 vel, out int maxHeight)
    {
        ...
        maxHeight = 0;
        while (ShouldIterate(pos) && !IsWithinArea(pos))
        {
            pos += vel;
